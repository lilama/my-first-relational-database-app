<?php
	...
	if (count($error) > 0) {
		$string = "../vue/creationSociete.php?status=false";
		foreach ($error as $cle=>$valeur) {
			$string .= "&" . $valeur . "=false";
			if ($cle !== 'status_non_valide') {
		...
		require "../modele/creationSocieteTrue.php";
		header("Location: ../vue/creationSociete.php?status=true");	
	}
?>